...
this.LOCAL_STORAGE_KEY="kgde_font_size";this.CSS_VAR_NAME="--root-font-size";this.render();this.addEventListener("click",this.setFontSizeOnClick);document.addEventListener("DOMContentLoaded",()=>{let value=window.localStorage.getItem(this.LOCAL_STORAGE_KEY);if(value!=null){this.state.fontSize=value;this.setFontSize();}});}
CSS(){return `<style>
    ::host {
...
setFontSizeOnClick(event){let buttons=event.composedPath().filter((element,index)=>element.tagName=="BUTTON");let currentSize=this.state.fontSize;if(buttons.length>0){let btn=buttons[0];let action=btn.dataset.action;if(action=="increase"){this.state.fontSize=parseFloat(currentSize)+0.1;}else{this.state.fontSize=parseFloat(currentSize)-0.1;}
